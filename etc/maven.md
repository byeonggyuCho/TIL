...
프로젝트 산출물을 빌드 테스트 디플로이하는 표준 라이프 사이클을 정의하고 이를 따르는 프로젝트에 공통적인 빌드로직을 쉽게 재사용할 수 있게 해주는 프레임워크
...
- default 라이프 사이클은 jar를 생성하는 작업들을 ㄱ수행하며, 다음과 같은 goal들로 구성된다.
...
### 1.6 의존성 관리
    - 종속된 라이브러리를 찾아서 등록해준다. (라이브러리간 의존성 관리)
<br><br><br>
...
**빌드절차**<br>
1. compile : 소스 파일 컴파일
2. test-compile : 단위 테스트 소스 파일 컴파일
3. test : 단위 테스트
4. package : 클래스 파일 패키지
5. integration-test : 통합테스트
6. install : artifact를 레포지토리로 복사
7. deploy : artifact를 리모트 레포지토리를 업로드
~~~
    mvn compile
...
- clean : 빌드된 결과물을 제거한다.
- compile: 소스 코드 컴파일
- test : Junit이나 TestNG 혹은 정적분석 도구와 함께 단위테스트를 수행한다. (테스트 실패시 빌드 실패)
- package : 배포가능한 형태 (JAR, WAR)로 컴파일 코드를 패키징
- integration-test : 통합테스트를 진행하고 필요할 시 패키지를 디플로이한다.
- install : 로컬 머신의 다른 프로젝트에서 종속 라이브러리로 사용될 수 있도록 패키지를 로컬 저장소에 설치한다.
- deploy : 통합환경 또는 릴리즈 환경에서 다른 개발자들 혹은 프로젝트들과 공유할 수 있도록 최종 패키지를 원격 저장소에 복사한다.
