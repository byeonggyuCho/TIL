...
- XSS 공격을 방어할 수있다.
- 이 경우 쿠키가 한정된 도메인에서만사용이 된다.  
- 이 단점은 토큰이 필요해질 때 현재 쿠키에 있는 토큰을 사용하여 새 토큰을 문자열로 받아올 수 있게 하는 API를 구현하면 된다
- CSRF 공격의 위험성이 생긴다.  
- CSRF는 HTTP 요청 레퍼러 체크, CSRF 토큰을 사용해서 방지할 수 있습니다.  
...
- [조대협블로그](https://bcho.tistory.com/999)