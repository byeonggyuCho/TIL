...
그러는 와중에 2005년 웹 구조의 판도를 바꾸는 기술이 등장했습니다. 구글 맵스(ajax를 이용한 애플리케이션)가 나오면서 정적 웹에서 벗어나 동적 웹의 시대가 도래했죠. ajax를 쓰면 서버에서 페이지를 완전히 Re-rendering할 필요가 없었습니다. 그 대신 클라이언트 측 애플리케이션 서버 APi와 Rest 앤드포인트에 직접 데이터를 요청하고 DOM을 동적으로 수정했습니다. 이 시점을 기점으로 웹에 대한 인식이 완전히 바뀌었죠.
...
MVC 구조에서 프로그램의 모든 객체는 모델, 뷰, 컨트롤러 중 하나 이어야합니다. 사용자는 view와 Interaction을 하고 View는 Model에 있는 데이터를 보여줍니다. controller는 이런 Interaction을 감시하며 필요할 때 view와 Model을 업데이트해야합니다.
